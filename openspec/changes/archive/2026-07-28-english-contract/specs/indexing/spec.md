...
(Previously: Spanish `omitidos`, `convencion.modo`, `libre`/`estricto`, reason string "el documento no tiene contenido indexable")
...
(Previously: Spanish `convencion.modo: "libre"`)
...
(Previously: Spanish `convencion.modo: "estricto"`, `convencion.tipos`/`convencion.estados`)
...
(Previously: Spanish `tipo`/`estado`/`modulo`)
...
(Previously: Spanish `titulo`, `estricto`)
...
(Previously: Spanish `titulo`/`resumen`/`modulo`/`tipo`/`estado`)
...
- GIVEN a file at `docs/my-guide_getting-started.md` with no H1
- WHEN indexed under `loose`
- THEN `title` resolves to `"My guide getting started"` (strip `.md`, replace `-`/`_` with spaces, collapse and trim whitespace, sentence-case the first letter)
...
(Previously: Spanish `tipo`/`modulo`/`estado` columns)
...
(Previously: Spanish `ruta`, `erroresLectura`, `omitidos`)
...
(Previously: Spanish `convencion.modo: "estricto"`, `ruta`, `omitidos`)
...
(Previously: Spanish `erroresLectura`, `ruta`, `omitidos`)
