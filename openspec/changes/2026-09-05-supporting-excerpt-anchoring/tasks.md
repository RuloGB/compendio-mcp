...
- [ ] 1.1 Create `scripts/supporting-anchor-probe.mjs`, importing only from `dist/`: `createContainer`
...
- [ ] 1.2 Implement goldenset reading (`<root>/goldenset.yaml` default, `pregunta` key, `// es-frozen:`
      comment) and a `--query "<text>"` override that, when given at least once, skips the goldenset
      entirely.
- [ ] 1.3 Implement **C7** first, before any other measurement: for each supporting fragment (rank ≥
...
- [ ] 1.4 Implement C1 (zero query terms visible via `foldForMatch` substring check), **C2 computed
...
- [ ] 1.5 **Binding amendment (Review response)**: emit an ordered digest of `(query, rank, path,
...
- [ ] 1.6 Implement the self-checks in this exact order, never conflated: `C7 > 0` →
...
- [ ] 1.7 Confirm the binding parameters are wired correctly: index mode hybrid (`node dist/cli.js
...
- [ ] 1.8 `npm run build`, then `node dist/cli.js --root ejemplos index` (hybrid, once — this database
...
- [ ] 1.9 Run Gate A baseline on unmodified code: `node scripts/supporting-anchor-probe.mjs ejemplos`.
...
- [ ] 2.1 In `test/application/search-documents-spans.test.ts`, rewrite the header comment (`:15-19`)
      ...
      file.
- [ ] 2.2 Invert the test body (`:37-68`, currently "a supporting (non-rank-0) result's excerpt stays
...
- [ ] 2.3 Invert `test/application/index-and-search.test.ts:185-204` (canary #2). `+1` → `+2` at
      ...
      assertion.
- [ ] 2.4 Run `npx vitest run test/application/search-documents-spans.test.ts
...
- [ ] 3.1 Append a **new `describe` block** to `test/application/search-documents-spans.test.ts`
...
- [ ] 3.2 Test: a supporting fragment whose flattened match sits past character 120 and short of the
      ...
      with `…`).
- [ ] 3.3 Test: a chunk with a high-frequency term near the start and a distinctive term clustered
...
- [ ] 3.4 Test pair, built so it can fail (Decision 7): a **control** document whose term occurs in
...
- [ ] 3.5 Run `npx vitest run test/application/search-documents-spans.test.ts` against **unmodified**
...
- [ ] 4.1 Edit `src/application/search-documents.ts:123`: `const spans = rank === 0 ?
      locateSpans(chunk.content, terms) : [];` → `const spans = locateSpans(chunk.content, terms);`.
- [ ] 4.2 Replace the comment at `:119-122` with design Decision 1's text verbatim (states the
...
- [ ] 4.3 Run `npx vitest run test/application/search-documents-spans.test.ts
...
- [ ] 5.1 `npm run build`. **Do NOT reindex** — the `ejemplos/.compendio` database from Phase 1.8 is
      reused; excerpts are query-time, so the same database is valid for both runs.
- [ ] 5.2 Re-run `node scripts/supporting-anchor-probe.mjs ejemplos`. **Require**: C1 = 0, C2 = 88
      (identical population), C7 = 0, exit 0. Record C3 ≈ 69 (78.4%), C4 ≈ 4.00, C5 ≈ 8 (9.1%), C6 = 0
      in `verify-report.md`.
- [ ] 5.3 **Binding amendment (Review response)**: diff this run's digest against
...
- [ ] 5.4 Re-run Gate B after the fix (same nonsense-query invocation as 1.8): confirm it still exits
      1 with `GATE IS VACUOUS`, C2 = 0 — the anti-vacuity guard fires identically in both tree states.
- [ ] 5.5 Run `node dist/cli.js --root ejemplos eval`. **Require identity, not a tolerance band**:
...
- [ ] 5.6 Measure C7's underlying shape directly against `ejemplos/.compendio/compendio.db` (count of
...
- [ ] 6.1 Edit `src/server.ts:119-121`'s **middle string literal only** — replace "the rest carry
...
- [ ] 6.2 Edit `AGENTS.md`'s MCP tools §2 clause: replace "...the rest get
...
- [ ] 6.3 Edit `AGENTS.md`'s "excerpt budget is graduated by rank" bullet: state first what did NOT
...
- [ ] 6.4 Add a new "Manual gate (`supporting-excerpt-anchoring`)" section to `AGENTS.md`'s Commands
      ...
      before/after table.
- [ ] 6.5 Add the `matchedTerms` deferred-follow-up bullet to `AGENTS.md` verbatim, per design
      Decision 8 (deferral count: 1, the `isFenceDelimiter` precedent).
- [ ] 6.6 Fix exactly 3 stale `Decision 7` comments: `src/domain/excerpt.ts:49`,
      ...
      do not re-touch `:331`.
- [ ] 6.7 Run `npm test`, `npm run typecheck`, `npm run build`. **Require**: 911 + 3 passing (914), 1
      skipped, 52 files.
- [ ] 6.8 `git diff --stat main..HEAD -- src/domain/flatten-map.ts src/domain/match-location.ts
      src/domain/ports.ts src/domain/fusion.ts src/infrastructure` → empty output, confirming design's
      asserted non-touches.
- [ ] 6.9 Confirm and record in `verify-report.md`, as two separate numbers per design Decision 7:
