...
Every indexed document MUST keep a root-prefixed `path`: the declared-root basename in explicit mode, and the discovered-root basename in discovery mode. Persisted and MCP-facing paths MUST match. `path` MUST remain the sole document identity key (`documents.path TEXT UNIQUE`); this introduces no schema change. Two documents that would otherwise share a relative path under different roots (e.g. `docs/a.md` and `openspec/a.md`) MUST both index successfully as distinct documents.
(Previously: discovery-mode paths were not specified, and the requirement assumed a default single-element `["docs"]` explicit root set — since retired in favor of discovery mode selecting roots dynamically when `docsDir` is absent, omitted, or `[]`; see Configuration spec.)
...
- GIVEN `docsDir: ["docs"]`, a file whose `path` is `docs/auth/login.md`, and frontmatter `module: ""`
- WHEN indexed under `loose`
...
#### Scenario: `module` from folder segment, single declared root

- GIVEN `docsDir: ["docs"]` and a file whose `path` is `docs/auth/login.md`
- WHEN indexed under `loose`
...
- GIVEN `docsDir: ["docs"]`, a file whose `path` is `docs/auth/login.md`, and frontmatter `module: "identity"`
- WHEN indexed under `loose`
...
- GIVEN `docsDir: ["docs"]` (a single declared root) and `docs/` cannot be read
- WHEN `compendio index` runs
