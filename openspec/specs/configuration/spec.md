...
The system MUST treat `docsDir` as the authoritative explicit root list when it is present as an array of strings in `compendio.config.json`. There is no single-string form and no compatibility fallback: when `docsDir` is malformed or wrong-typed, configuration loading MUST fail and MUST NOT fall back to discovery mode or any hidden default. When `docsDir` is absent, omitted, or declared as `[]`, the system MUST switch to discovery mode instead of inventing a default root list (see "Missing `docsDir` or Empty `docsDir` Enables Discovery Mode" below). In explicit mode, each declared entry becomes one root; every discovered document `path` MUST be prefixed with that root's alias — the basename of the declared root's path (see Indexing spec for the prefixed-`path` contract). Aliases MUST NOT be a project-declarable config key.
(Previously: absent `docsDir` defaulted to `["docs"]`; empty arrays were rejected at construction; discovery mode did not exist.)
...
- WHEN `compendio index` runs
- THEN every discovered `path` is prefixed with the `docs` alias (`docs/x.md`), identical in shape to the discovery-mode default
...
### Requirement: Colliding, Nested, or Duplicate Declared Root Sets Are Rejected at Construction

The system MUST reject the configuration — throwing before any document is discovered, chunked, or written to the index — when any of: (a) two declared roots resolve to the same absolute path, including a case-differing duplicate on a case-insensitive filesystem; (b) one declared root's absolute path is nested inside another declared root's absolute path, checked as an ordered pair in BOTH declaration orders (the outer root declared first, and the outer root declared second/inner-first); (c) the array contains duplicate entries; or (d) two declared roots derive the same alias (basename collision). This validation applies only to a non-empty declared `docsDir` array — an empty array is not a rejection case; it selects discovery mode instead (see "Missing `docsDir` or Empty `docsDir` Enables Discovery Mode"). The thrown error MUST name the offending root(s). This validation MUST run at config load or container construction time, strictly before any existing index is reset or modified, and strictly before any `.compendio/` directory is created for a fresh project.
(Previously: included a fifth rejection case, an empty declared array; superseded because an empty `docsDir` now selects discovery mode instead of being rejected.)
...
#### Scenario: Directory-prefix exclude works against a single declared root too

- GIVEN `docsDir: ["docs"]` and `exclude: ["docs/archive"]`
- WHEN a file at `docs/archive/old.md` is discovered
- THEN it is excluded, along with every other file under `docs/archive/`
(Previously: this scenario relied on `docsDir` defaulting to `["docs"]`; superseded because a genuinely absent `docsDir` now selects discovery mode instead of an implied `["docs"]` root — the same directory-prefix rule still applies once `docs` is explicitly declared.)
