...
- **An unreadable explicit root keeps its rows; discovery mode fails closed before any store mutation, except a root whose `lstat` reports `ENOENT`, which is purged.** Removing a root from `docsDir` purges its documents on the next sync. `composite-document-source.ts`, `sync-index.ts` — [more](docs/design-decisions.md#unreadable-roots-fail-closed-in-discovery-mode)
