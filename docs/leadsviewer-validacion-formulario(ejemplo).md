...
- Se registran fecha y hora (UTC) de la aceptación junto al lead, como evidencia del consentimiento.
...
1. **Alta de lead válido.** El usuario rellena Nombre, Apellidos, Email y Origen, marca el consentimiento y guarda. El sistema normaliza los datos, registra la fecha del consentimiento y muestra la ficha del nuevo lead.
...
- Importación masiva de leads (CSV u otras fuentes); se documentará por separado.
...
- Contrato de API de alta y edición de leads: pendiente de redacción (`api/leadsviewer-alta-leads.md`).
